# Makefile generated by MakefileGenerator.cs
# *DO NOT EDIT*

UNREALROOTPATH = /home/forklovee/dev/UnrealEngine54
GAMEPROJECTFILE =/home/forklovee/dev/UnrealEngine54/WheelMealMania/WheelMealMania.uproject

TARGETS = \
	UnrealEditor-Linux-DebugGame  \
	UnrealEditor-Linux-Shipping  \
	UnrealEditor \
	UnrealGame-Linux-DebugGame  \
	UnrealGame-Linux-Shipping  \
	UnrealGame \
	WheelMealMania-Linux-DebugGame  \
	WheelMealMania-Linux-Shipping  \
	WheelMealMania \
	WheelMealManiaEditor-Linux-DebugGame  \
	WheelMealManiaEditor-Linux-Shipping  \
	WheelMealManiaEditor\
	configure

BUILD = bash "$(UNREALROOTPATH)/Engine/Build/BatchFiles/Linux/Build.sh"
PROJECTBUILD = "$(UNREALROOTPATH)/Engine/Binaries/ThirdParty/DotNet/6.0.302/linux/dotnet" "$(UNREALROOTPATH)/Engine/Binaries/DotNET/UnrealBuildTool/UnrealBuildTool.dll"

all: StandardSet

RequiredTools: CrashReportClient-Linux-Shipping CrashReportClientEditor-Linux-Shipping ShaderCompileWorker UnrealLightmass EpicWebHelper-Linux-Shipping

StandardSet: RequiredTools UnrealFrontend WheelMealManiaEditor UnrealInsights

DebugSet: RequiredTools UnrealFrontend-Linux-Debug WheelMealManiaEditor-Linux-Debug


UnrealEditor-Linux-DebugGame:
	 $(BUILD) UnrealEditor Linux DebugGame  $(ARGS)

UnrealEditor-Linux-Shipping:
	 $(BUILD) UnrealEditor Linux Shipping  $(ARGS)

UnrealEditor:
	 $(BUILD) UnrealEditor Linux Development  $(ARGS)

UnrealGame-Linux-DebugGame:
	 $(BUILD) UnrealGame Linux DebugGame  $(ARGS)

UnrealGame-Linux-Shipping:
	 $(BUILD) UnrealGame Linux Shipping  $(ARGS)

UnrealGame:
	 $(BUILD) UnrealGame Linux Development  $(ARGS)

WheelMealMania-Linux-DebugGame:
	 $(PROJECTBUILD) WheelMealMania Linux DebugGame  -project="$(GAMEPROJECTFILE)" $(ARGS)

WheelMealMania-Linux-Shipping:
	 $(PROJECTBUILD) WheelMealMania Linux Shipping  -project="$(GAMEPROJECTFILE)" $(ARGS)

WheelMealMania:
	 $(PROJECTBUILD) WheelMealMania Linux Development  -project="$(GAMEPROJECTFILE)" $(ARGS)

WheelMealManiaEditor-Linux-DebugGame:
	 $(PROJECTBUILD) WheelMealManiaEditor Linux DebugGame  -project="$(GAMEPROJECTFILE)" $(ARGS)

WheelMealManiaEditor-Linux-Shipping:
	 $(PROJECTBUILD) WheelMealManiaEditor Linux Shipping  -project="$(GAMEPROJECTFILE)" $(ARGS)

WheelMealManiaEditor:
	 $(PROJECTBUILD) WheelMealManiaEditor Linux Development  -project="$(GAMEPROJECTFILE)" $(ARGS)

configure:
	xbuild /property:Configuration=Development /verbosity:quiet /nologo "$(UNREALROOTPATH)/Engine/Source/Programs/UnrealBuildTool/UnrealBuildTool.csproj"
	$(PROJECTBUILD) -projectfiles -project="\"$(GAMEPROJECTFILE)\"" -game -engine 

.PHONY: $(TARGETS)
